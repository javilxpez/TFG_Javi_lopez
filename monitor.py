...
def render(data: dict, meta: dict):
    ...
    rows.append(("LC Valid",    f"{'Yes' if lc_valid else 'No'}"))
    rows.append(("", ""))
    ...
    lines.append(f"  Reads: {meta['count']}  │  {meta['rate']:.0f} Hz  │  CRC err: {meta['crc_err']}  │  Sync err: {meta['sync_err']}")
    lines.append("─" * 56)
...
def main():
    ...
    if not port:
        print("No serial port found. Pass port as argument.")
        sys.exit(1)
    ...
    meta = {"port": port, "count": 0, "crc_err": 0, "sync_err": 0, "rate": 0.0}
    rate_window = []
    last_data = {}
