...
Formato común: \codigo{0xAA 0x55 LEN carga\_útil[LEN] CRC8}. Enteros en \emph{little-endian}. CRC-8
con polinomio 0x07 y valor inicial 0x00 sobre la carga útil. Los desplazamientos se dan en bytes
dentro de la carga útil.
...
+6 & ref\_cmd & i16 & Consigna de velocidad escrita (rpm); negativa hacia A \\
+8 & base\_read & i16 & Lectura filtrada menos tara (cuentas) \\
+10 & io & u8 & b0: final A (arriba); b1: final B (abajo) \\
...
3 & gain & u8 & Ganancia grabada (código 0--7, no lineal: sección~\ref{sec:prob-estado}) \\
...
\section{Paquete 0x05: estado del ciclo (18 bytes)}

\noindent\begin{tabularx}{\textwidth}{@{}rllX@{}}
\toprule
0 & packet\_id & u8 & 0x05 \\
1 & phase & u8 & 0 reposo, 1 colocándose en B, 2 hacia A, 3 volviendo a B, 4 completado, 5 abortado \\
2 & done & u16 & Repeticiones completadas \\
4 & target & u16 & Repeticiones pedidas (0 = sin fin) \\
6 & pos\_mrev & i32 & Posición actual × 1000 \\
10 & target\_mrev & i32 & Recorrido B$\rightarrow$A pedido × 1000 \\
14 & pos\_b\_mrev & i32 & Posición en la que ha quedado fijado B × 1000 \\
\bottomrule
\end{tabularx}

\medskip
\noindent La versión anterior de este paquete ocupaba 14 bytes y no llevaba el último campo. El
servidor acepta las dos longitudes: con la trama corta asume \codigo{pos\_b\_mrev = 0}, de modo que
un firmware y un servidor desparejados siguen informando de la fase
(sección~\ref{sec:prob-ciclo}).
...
MOVE\_A / MOVE\_B & \codigo{04} / \codigo{05} & A es arriba; B, abajo \\
SHUTDOWN & \codigo{06} & \\
...
CYCLE\_START & \codigo{0C mm tt tt vv vv rr rr} & mm = 0: hasta el final de carrera; 1: hasta la distancia indicada. Recorrido i16 (centi-rev), velocidad i16, repeticiones u16 \\
CYCLE\_STOP & \codigo{0D} & \\
\bottomrule
\end{tabularx}

\medskip
\noindent Atajos de terminal (sólo con el decodificador en reposo): \codigo{i} INIT, \codigo{a}
MOVE\_A, \codigo{b} MOVE\_B, \codigo{x} STOP, \codigo{h} HOME, \codigo{q} SHUTDOWN.
